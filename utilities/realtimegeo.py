...
# Scrape some instagram data by location and time.  See baltimore.sh as an example of how to run.
...
spread = 0.085 # this is appropriate lat/lon difference given the 5k max below
...
while realtime:
  ...
  while tmp_lat <= maxlat and tmp_lon <= maxlon:
    ...
    print "**** Starting:", tmp_lat, "", tmp_lon, tmp_start_date, "-", end_date
    ...
    while tmp_start_date < end_date:
      ...
      bykey = {}
      for img in j['data']:
        q.put({"id":img['id'], "url":img['images']['thumbnail']['url']})
        indexline = { "index" : { "_index" : "instagram", "_type" : direct, "_id" : img['id'] } }
        dataline = img
